...
STARTUP:		jmp	_VDU_INIT_MODE			; Initialise screen with mode in A.
...
_BC4EF:			tay					; Y=A
			...
			lsr					; 
			clc					; clear carry
			adc	#VDU_DRIVER_PAGE		; add &C3 to get hi byte of link address
			sta	VDU_JUMPVEC_HI			; 
...
__vdu_mode_init_loop:	sta	VDU_VARS_BASE-1,X		; Zero VDU workspace at &300 to &37E
			dex					
...
_BCB5E:			asl					; A=A*2
			...
			ldy	_TAB_MAP_TYPE,X			; screen display memory index table
			...
			lsr					; /2
			tax					; X=A
			lda	_TAB_MULTBL_LKUP,X		; row multiplication table pointer
			sta	VDU_ROW_MULT			; store it
			lda	#>_MUL640_TABLE			; A=&C3
...
_LCBC1_DOCLS:		ldx	#$00				; X=0
			...
			ldy	_TAB_CLS_ENTER,X		; get section control number
			sty	VDU_JUMPVEC			; set it in jump vector lo
			ldy	#>_VDU_CLEAR_20K		; Y=&CC
...
;************************ Mode 0,1,2 entry point *************************
	...
	.assert >_VDU_CLEAR_20K = >_VDU_CLEAR_1K, error, "CLS routine entry points must all be in same page"

_VDU_CLEAR_20K:		sta	$3000,X				; 
			sta	$3100,X				; 
			sta	$3200,X				; 
			sta	$3300,X				; 
			sta	$3400,X				; 
...
_LCDE2:			ldx	#<VDU_G_CURS_H			; point to graphics cursor
_LCDE4:			ldy	#<VDU_G_CUR_YX_HI+1		; point to last graphics cursor
...
_LD42C:			lda	VDU_TMP5			; A=&DE
			...
			ldx	#<VDU_BITMAP_READ		; X=&28
			ldy	#<VDU_BITMAP_RD_2		; Y=&2A
			jsr	_LCDDE_EXG2_P3				; exchange 300/1+Y with 300/1+X
								; IN THIS CASE THE X AND Y SPANS!

_BD437:			ldx	#<VDU_BITMAP_READ		; X=&28
			ldy	#<VDU_WORKSPACE+7		; Y=&37
...
_LD5EA:			ldx	#<VDU_QUEUE_5			; X=&20
			ldy	#<VDU_WORKSPACE+$E		; Y=&3E
			...
			ldx	#<VDU_G_CUR_YX_HI+1		; X=&14
			ldy	#<VDU_G_CURS_H			; Y=&24
			jsr	_LD636				; 
			jsr	_LD632				; 
			...
			ldx	#<VDU_BITMAP_READ		; set pointers
			jsr	_LD459				; 
			ldy	#<VDU_BITMAP_RD_6		; 
			...
			ldx	#<VDU_WORKSPACE+$E		; ;X=&3E
			ldy	#<VDU_QUEUE_5			; ;Y=&20
			jsr	_LD47C				; ;copy 300/7+X to 300/7+Y
			jmp	_LD0D9				; ;this gets XY data parameters and current graphics
								; cursor position

_LD632:			ldx	#<VDU_QUEUE_5			; X=&20
			ldy	#<VDU_G_CUR_YX_HI+1		; Y=&14
